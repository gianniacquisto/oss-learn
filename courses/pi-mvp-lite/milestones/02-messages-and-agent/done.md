# Done Checklist — Milestone 2: Messages & Agent State
...
- All message types (`UserMessage`, `AssistantMessage`, `ToolResultMessage`, `SystemMessage`) are properly defined in `types.ts`
...
- Streaming responses or event emission — event emission comes in Milestone 3, streaming in Milestone 5
...
### "Cannot find module './agent.js'"
- Make sure you ran `npm run build` first. The CLI runs from `dist/`, not `src/`. If `dist/agent.js` doesn't exist, the import will fail.
