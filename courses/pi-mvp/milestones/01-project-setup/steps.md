# Steps for Milestone 1: Project Setup & Configuration
...
- Define a `AgentConfig` interface with these fields:
  ...
  - `systemPrompt?: string` — optional system prompt override (uses default if not provided)
  - `thinkingLevel?: "off" | "low" | "medium" | "high"` — reasoning level for supported models
- Implement a `loadConfig()` function that:
  1. Checks for an optional `.agentrc.json` file in the current directory
  2. Falls back to environment variables (`AGENT_PROVIDER`, `AGENT_API_KEY`, `AGENT_MODEL`) if no config file exists
  3. Uses sensible defaults if neither is provided (e.g., provider = `"anthropic"`, thinkingLevel = `"off"`)
- The function should throw a descriptive error if the API key is missing and can't be resolved
...
**Reference pattern:**
```typescript
// config.ts — structural reference only
...
- Keep the CLI minimal here — you'll add real command parsing in later milestones when you have more to do.

**Reference pattern:**
```typescript
// cli.ts — structural reference only
